...
Current published version: **0.2.0** (GitHub Packages). The repository may track a newer **0.2.1-SNAPSHOT** between releases; dependency examples below use **0.2.0** unless you deliberately depend on snapshots from `mavenLocal()`.
...
To depend on published artifacts (replace the version with the tag you target, e.g. **0.2.0**), add the GitHub Packages repository and the dependency coordinates below. Choose Gradle or Maven and follow that section from top to bottom.
...
```kotlin
dependencies {
    implementation("it.battlejar:battlejar-api:0.2.0")
    implementation("it.battlejar:battlejar-client:0.2.0")
    // optional: only if you need Vector2 and other math utilities
    implementation("it.battlejar:battlejar-math:0.2.0")
}
```
